...
description: Wraps dev-pipeline with post-merge oversight -- verifies ALL PR comments (CodeRabbit, review skill, manual) are addressed, not just the ship agent's own findings. Adapted for Northwoods.
...
> - **MCP tools** (`search`, `repo_map`) -- project-level context: guardrails, outcomes, cross-cutting impact.
> - **CLI** -- code within your working directory:
>   ```bash
>   repo-native-alignment search --repo . "query" --limit 5
>   repo-native-alignment graph --node "file:symbol:kind" --repo . --mode neighbors
>   ```
>
> **MANDATORY scan before any code navigation in Phase 5.** Verify the index:
> ```bash
...
> ```
>
> **Friction consequence:** Any Grep/Read used for code navigation after the scan has run is a `skipped` friction event. Log it to `.oh/friction-logs/<original-pr>-oversight.md`.
...
5. **Create a followup PR if needed:**
   ...
   - Run `pnpm check` and relevant tests before pushing
   - Wait for CodeRabbit to review the followup PR
   - Address any new CodeRabbit findings on the followup
...
- [ ] **Tenant isolation** -- verify any tenant-scoping concerns are addressed (Northwoods guardrail)
...
- [ ] **Extraction pipeline** -- verify extraction provider changes don't break append-only attempt audit trail

### Delivery spot-check

**After the comment audit, verify the feature actually works end-to-end.**

1. Read the PR's ship step delivery verification comment
2. If the feature touches the intake/review workflow, run a live smoke test:
   ```bash
   curl -sS -F 'file=@samples/intakes/chatgpt-sample-general-intake.pdf' \
     -F 'templateId=general-assistance' -H 'X-Tenant-Id: tenant-a' \
     http://localhost:5100/intakes
   ```
3. If the feature doesn't produce visible output in real usage, **flag it** and create a fix
...
**Adds:** Phase 5 (post-merge comment audit + delivery spot-check)
**Use instead of:** `dev-pipeline` when the PR is likely to get external review comments (complex changes, API changes, tenant-scoping work)
