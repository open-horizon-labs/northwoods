...
public static class DatabaseInitializer
{
    ...
    {
        // Maps template IDs to blank v2-layout PDFs in the samples/intakes directory.
        // Generated by scripts/corpus/gen_blank_templates.py.
        var templatePdfMap = new Dictionary<string, string>
        {
            ["general-assistance"] = "blank-general-assistance.pdf",
            ["housing-stability"] = "blank-housing-stability.pdf",
            ["behavioral-health"] = "blank-behavioral-health.pdf",
            ["soap-note"] = "blank-soap-note.pdf",
        };
    ...
    private const string CleanupMockDataSql = """
        ...
        -- Includes both old chatgpt-sample-* names and new blank-* names for backwards compat.
        DELETE FROM audit_events WHERE document_id IN (
            SELECT id FROM documents WHERE original_file_name IN (
                ...
                'blank-general-assistance.pdf',
                'blank-housing-stability.pdf',
                'blank-behavioral-health.pdf',
                'blank-soap-note.pdf'
            )
        );
